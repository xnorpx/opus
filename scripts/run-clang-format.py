...
def main():
    ...
    try:
        version = subprocess.check_output(version_invocation)
    ...
    version = re.findall("clang-format version (\d+.\d+.\d+)",
                         version.decode('utf-8'))[0]
    if version != args.clang_format_version:
        print_trouble(
            parser.prog,
            "Clang-format version '{}' is not the same as enforced '{}' version.".format(
                version, args.clang_format_version
            ),
            use_colors=colored_stderr,
        )
        return ExitStatus.TROUBLE
